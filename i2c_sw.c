...
void scl_in_mode(uint8_t sel)
{
    GPIO_InitTypeDef GPIO_InitStructure;
		...
		GPIO_InitStructure.GPIO_Speed=GPIO_Speed_50MHz;
		GPIO_InitStructure.GPIO_Mode=GPIO_Mode_Out_OD; //пїЅпїЅпїЅпїЅпїЅпїЅ out? пїЅпїЅпїЅпїЅпїЅпїЅ пїЅпїЅ пїЅпїЅпїЅпїЅпїЅпїЅ
		
    if(sel == 1)
    {
        GPIO_InitStructure.GPIO_Pin   = SW_I2C2_SCL_PIN;
        HAL_GPIO_Init(SW_I2C2_SCL_GPIO, &GPIO_InitStructure);
    }
    //else if(sel==x) ...
}
...
uint8_t SW_I2C_WriteControl_8Bit(uint8_t sel, uint8_t IICID, uint8_t regaddr, uint8_t data)
{
    ...
    if (!SW_I2C_ReadVal_SDA(sel))    	return FALSE;
    if (!SW_I2C_ReadVal_SCL(sel))    	return FALSE;
